...
<body>
        ...
        <section id="categories" class="pt-3">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <a href="{{route('index')}}"><i class="fa-solid fa-house"></i> &nbsp;>&nbsp; </a> <a href="{{route('AllSubjects.get')}}">Subjects > </a> <a href="{{route('books.all')}}"> Books</a>
                    </div>
                    <div class="col-12">
                        <div class="mt-3">
                                        ...
                                        @forelse ($AllBooks as $Book)
                                            <article class="subject px-3 rounded-3">
                                                <div class="subject-main">
                                                    <header>
                                                        <h3 class=""><a href="{{route('books.single', $Book->id)}}">{{ $Book->title }}</a></h3>
                                                    </header>
                                                    @if ($Book->author->name != 'Deleted Author')
                                                    <p>{{ $Book->author->name }}</p>
                                                    @endif
                                                    @if ($Book->Category->title != 'Deleted Category')
                                                    <p>Category: <b><a href="">{{ $Book->Category->title }}</a></b></p>
                                                    @endif
                                                </div>
                                                <div class="subject-photo">
                                                <img class="rounded-2" src="{{ $Book->BookImage }}" alt="">
                                                </div>
                                            </article>
                                        @empty
                                        @endforelse
