...
<body>
        ...
        <section id="categories" class="pt-3 mb-4">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <a href="{{route('index')}}"><i class="fa-solid fa-house"></i> &nbsp;>&nbsp; </a> <a href="{{route('AllSubjects.get')}}">Subjects > </a> <a href="{{route('books.all')}}"> Audios &nbsp; > </a> <span>&nbsp; {{$TheAudio->title}}</span>
                    </div>
                    <div class="col-12">
                        <div class="mt-3">
                            <div class="row">
                                <div class="col-8">
                                    <article class="book py-3 rounded-3">
                                        <div class="article-main">
                                            <h3 class=""><a href="">{{ $TheAudio->title }}</a></h3>
                                            <audio controls>
                                                {{-- <source src="horse.ogg" type="audio/ogg"> --}}
                                                <source src="{{$TheAudio->AudioFile}}" type="audio/mpeg">
                                            Your browser does not support the audio element.
                                            </audio>
                                            @if ($TheAudio->author->name != 'Deleted Author')
                                            <p>{{ $TheAudio->author->name }}</p>
                                            @endif
                                            @if ($TheAudio->Category->title != 'Deleted Category')
                                            <p>Category: <b><a href="">{{ $TheAudio->Category->title }}</a></b></p>
                                            @endif
                                            <p><i class="fa-solid fa-calendar-days"></i> {{$Date}}</p>
                                        </div>
                                        <a class="main-btn mt-3" href="{{$TheAudio->AudioFile}}" download="{{$TheAudio->AudioFile}}">Download <i class="fa-solid fa-download"></i></a>
                                    </article>
                                                ...
                                                <div class="single-blog-comment mb-40">
                                                    ...
                                                    <div class="blog-comment-content">
                                                        ...
                                                        <div class="comment-name-reply">
                                                            <h5>{{date('d F Y', strtotime($TheAudio->created_at))}}</h5>
                                                        </div>
                                                    </div>
                                                </div>
                                                @empty
                                    ...
                                    <div class="comment-form">
                                        ...
                                        <form action="{{route('comment.post', [$TheAudio->id, 'book'])}}" method="POST">
                                            @csrf
                                            <div class="row">
                                        ...
                                        <div class="col-3">
                                            <article class="book px-3 rounded-3 my-3">
                                                <div class="article-photo mb-3">
                                                    <img class="rounded-2" width="100px" src="{{ $Audio->AudioImage }}" alt="">
                                                    </div>
                                                <div class="article-main">
                                                    <h3 class="mb-0"><a href="{{route('books.single', $Audio->id)}}">{{ $Audio->title }}</a></h3>
                                                    @if ($Audio->author->name != 'Deleted Author')
                                                    <p>{{ $Audio->author->name }}</p>
                                                    @endif
                                                </div>
                                            </article>
                                        </div>
                                ...
                                <div class="col-4">
                                    <div class="most-readed bg-gray-6 p-3 rounded-3">
                                        ...
                                        <article class=" my-3">
                                            <div class="article-main">
                                                <h3 class="mb-0"><a href="">{{ $Audio->title }}</a></h3>
                                                @if ($Audio->author->name != 'Deleted Author')
                                                <p class="author">{{ $Audio->author->name }}</p>
                                                @endif
                                            </div>
                                        </article>
                                    @empty
